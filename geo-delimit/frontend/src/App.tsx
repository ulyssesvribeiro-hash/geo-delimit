...
// Converte a geometria de uma camada Leaflet.draw (polygon/rectangle) para GeoJSON
// fechando o anel corretamente (primeiro ponto = último ponto), exigido pelo PostGIS.
function leafletLayerToClosedGeoJSON(layer: any): GeoJSON.Geometry | null {
  try {
    ...
    if (geom.type === 'Polygon') {
      const closedRings = geom.coordinates.map((ring: number[][]) => {
        const first = ring[0];
...
export default function App() {
  ...
  const bairrosLayerRef = useRef<any>(null);
  const tileLayerRef    = useRef<any>(null);
  const drawnItemsRef   = useRef<any>(null);   // FeatureGroup que guarda o polígono desenhado/editado
  const drawControlRef  = useRef<any>(null);   // Controle de desenho do Leaflet.draw
  ...
  // ── Painel de resumo territorial + bairros sobrepostos no mapa ──
  ...
  const [bairrosCarregados, setBairrosCarregados] = useState(0);

  // ── Legenda fixa (resumo geral de siglas em uso) ──
  const [legendaAberta,   setLegendaAberta]   = useState(true);
  const [legendaDados,    setLegendaDados]    = useState<FaccaoResumo[]>([]);
  ...
  // ─── Carrega Leaflet + Leaflet.draw dinamicamente ─────────────────────────
  useEffect(() => {
    ...
    if (!document.getElementById('leaflet-js')) {
      ...
      script.onload = () => {
        ...
        if (!document.getElementById('leaflet-draw-js')) {
          ...
          drawScript.onload = () => setDrawReady(true);
      ...
      setLeafletReady(true);
      setDrawReady(!!(window.L as any).Draw);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    bairrosLayerRef.current = L.layerGroup().addTo(map);
  ...
  // ─── Reposiciona a toolbar de desenho para a direita do painel lateral ────
  // O Leaflet.draw posiciona seu controle fixo na margem esquerda do MAPA
  // (left: 10px), mas nosso painel lateral (340px) fica sobre essa mesma
  // margem com z-index mais alto, escondendo a toolbar visualmente mesmo
  // com ela presente e estilizada corretamente no DOM. Ajustamos o `left`
  // do container via JS sempre que o painel abre/fecha ou o modo desenho liga.
  useEffect(() => {
    if (!drawMode) return;
    const container = document.querySelector('.leaflet-top.leaflet-left') as HTMLElement | null;
    if (!container) return;
    container.style.left = sidebarOpen ? '356px' : '10px';
  }, [drawMode, sidebarOpen]);

  // ─── Captura os eventos do Leaflet.draw (criar/editar/excluir) ────────────
  useEffect(() => {
    const map = mapRef.current;
    const L = leafletRef.current;
    if (!map || !L || !drawReady || !drawnItemsRef.current) return;

    const drawnItems = drawnItemsRef.current;

    const syncGeometryFromDrawnItems = () => {
      ...
      const geom = leafletLayerToClosedGeoJSON(lastLayer);
      if (geom) {
    ...
    const onCreated = (e: any) => {
      ...
      applyStyleToDrawnLayer(e.layer, style);
      syncGeometryFromDrawnItems();
    };

    const onEdited = () => { syncGeometryFromDrawnItems(); };
    const onDeleted = () => { syncGeometryFromDrawnItems(); };
  ...
  function applyStyleToDrawnLayer(layer: any, s: AreaStyle) {
    if (layer && layer.setStyle) {
      try { layer.setStyle({ color: s.cor, weight: s.espessura, fillColor: s.cor, fillOpacity: s.opacidade }); } catch { /* ignorar */ }
    }
  }
  ...
  useEffect(() => {
    ...
    drawnItems.eachLayer((layer: any) => applyStyleToDrawnLayer(layer, style));
  }, [style]);

  // ─── Liga/desliga o modo de desenho livre ─────────────────────────────────
  const toggleDrawMode = useCallback(() => {
    const map = mapRef.current;
    const L = leafletRef.current;
    if (!map || !L || !drawReady) return;
    ...
    if (drawMode) {
      ...
      const mainLg = mainLayerRef.current;
      const bairrosLg = bairrosLayerRef.current;
      if (mainLg) mainLg.clearLayers();
      if (bairrosLg) bairrosLg.clearLayers();
      setTerritorioPanel(null);
      setBairrosCarregados(0);
      setMetrics(null);
      ...
      const drawControl = new L.Control.Draw({
        ...
        draw: {
          polygon: {
            shapeOptions: { color: style.cor, weight: style.espessura, fillColor: style.cor, fillOpacity: style.opacidade },
      ...
      setDrawMode(true);
      setDrawMsg('Use as ferramentas no canto superior esquerdo do mapa para desenhar ou editar o polígono.');
    }
  }, [drawMode, drawReady, style]);

  // ─── Limpa a forma desenhada manualmente ──────────────────────────────────
  ...
  // ─── Carrega a legenda (resumo de siglas em uso) ao iniciar o app ─────────
  const fetchLegenda = useCallback(async () => {
    setLegendaLoading(true);
    try {
      const res = await fetch(`${API_BASE}/areas/faccoes/resumo`);
      if (!res.ok) throw new Error('Erro ao buscar resumo');
      const data = await res.json();
      setLegendaDados(Array.isArray(data) ? data : []);
    } catch {
      setLegendaDados([]);
    } finally {
  ...
  // ─── Desenha o polígono principal (cidade ou bairro selecionado via busca) ─
  const drawMainPolygon = useCallback((geometry: GeoJSON.Geometry, s: AreaStyle) => {
    const L = leafletRef.current;
    const lg = mainLayerRef.current;
    if (!L || !lg) return;
  ...
  // ─── Carrega o polígono atual (de uma busca) para edição manual de vértices ─
  const editarPoligonoAtual = useCallback(() => {
    const L = leafletRef.current;
    const map = mapRef.current;
    const drawnItems = drawnItemsRef.current;
    if (!L || !map || !drawnItems || !currentGeometry) return;

    drawnItems.clearLayers();
    const layer = L.geoJSON(currentGeometry).getLayers()[0];
    if (!layer) return;

    applyStyleToDrawnLayer(layer, style);
  ...
  }, [currentGeometry, style, drawMode, toggleDrawMode]);

  // ─── Desenha os bairros faccionados sobrepostos (camada secundária) ───────
  const drawBairrosSobrepostos = useCallback(async (areas: SavedArea[]) => {
    const L = leafletRef.current;
    const lg = bairrosLayerRef.current;
    if (!L || !lg) { setBairrosCarregados(0); return; }

    lg.clearLayers();
    let carregados = 0;

    const results = await Promise.allSettled(
      areas.map(a => fetch(`${API_BASE}/areas/${a.id}`).then(r => r.ok ? r.json() : null))
    );

    results.forEach((res, idx) => {
      if (res.status !== 'fulfilled' || !res.value?.geometry) return;
      const area = areas[idx];
      const cor = area.cor_contorno || faccaoColor(area.faccao) || '#FF0000';
      const feature: GeoJSON.Feature = {
        type: 'Feature',
        properties: { nome: area.nome, faccao: area.faccao },
        geometry: res.value.geometry,
      };

      const layer = L.geoJSON(feature, {
        style: () => ({ color: cor, weight: 2.5, opacity: 0.95, fillColor: cor, fillOpacity: 0.18 }),
      }).addTo(lg);

      L.geoJSON(feature, { style: () => ({ color: cor, weight: 9, opacity: 0.12, fill: false }) }).addTo(lg);

      const label = area.faccao ? `${area.nome} — ${area.faccao.toUpperCase()}` : area.nome;
      layer.bindPopup(
    ...
    setBairrosCarregados(carregados);
  }, []);

  useEffect(() => {
    if (currentGeometry && !drawMode) drawMainPolygon(currentGeometry, style);
  }, [style, currentGeometry, drawMode, drawMainPolygon]);

  // ─── Animação de pulsação (apenas no polígono principal, fora do modo desenho) ─
  useEffect(() => {
    if (animFrameRef.current) { cancelAnimationFrame(animFrameRef.current); animFrameRef.current = null; }
    if (!style.animacao || !currentGeometry || drawMode) return;
  ...
  }, [style.animacao, style.espessura, currentGeometry, drawMode]);
  ...
  // ─── Consulta bairros já cadastrados em um município e desenha no mapa ────
  const fetchTerritorioDaCidade = useCallback(async (municipioNome: string) => {
    ...
    try {
      ...
      await drawBairrosSobrepostos(areas);
    } catch {
      setTerritorioPanel(null);
      setBairrosCarregados(0);
    } finally {
  ...
  // ─── Carrega uma área salva no mapa (camada principal) ────────────────────
  const handleLoadSavedArea = async (area: SavedArea) => {
    ...
    try {
      ...
      setCurrentUpdatedAt(full.data_atualizacao || full.data_criacao || null);
      setQuery(area.nome);
      setSelectedResult(null);
      ...
      drawMainPolygon(full.geometry, loadedStyle);
      ...
      if (area.municipio) fetchTerritorioDaCidade(area.municipio);
    } catch (err) {
  ...
  const selectAllVisible = () => {
    setSelectedIds(new Set(filteredAreasMemo.map(a => a.id)));
  };
  ...
  const handleBulkDelete = async () => {
    ...
    const results = await Promise.allSettled(
      ids.map(id => fetch(`${API_BASE}/areas/${id}`, { method: 'DELETE' }))
    );

    const succeededIds = ids.filter((_, i) => results[i].status === 'fulfilled');
    setSavedAreas(prev => prev.filter(a => !succeededIds.includes(a.id)));
    setTerritorioPanel(prev => prev ? prev.filter(a => !succeededIds.includes(a.id)) : prev);

    const failedCount = ids.length - succeededIds.length;
    setListError(failedCount > 0 ? `${failedCount} área(s) não puderam ser excluídas` : '');
  ...
  const handleSave = async () => {
    if (!currentGeometry) return setSaveMsg('❌ Desenhe ou selecione uma área primeiro');
    ...
    try {
      const res = await fetch(`${API_BASE}/areas`, {
        ...
        body: JSON.stringify({
          ...
          fonte_dados: hasDrawnShape ? 'desenho_manual' : 'openstreetmap',
        }),
      });
      if (!res.ok) throw new Error(await res.text());
      const created = await res.json();
      setCurrentAreaId(created.id || null);
      setCurrentUpdatedAt(created.data_criacao || new Date().toISOString());
  ...
  const filteredAreasMemo = savedAreas.filter(a => {
  ...
  const filteredAreas = filteredAreasMemo;
  ...
  return (
    <div style={{ height: '100vh', width: '100vw', background: mapTheme === 'dark' ? '#0a0a0f' : '#e8eaed', display: 'flex', flexDirection: 'column', fontFamily: "'Inter', sans-serif", overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 1000, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12, background: mapTheme === 'dark' ? 'linear-gradient(to bottom, rgba(0,0,0,0.9), transparent)' : 'linear-gradient(to bottom, rgba(255,255,255,0.9), transparent)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', color: mapTheme === 'dark' ? '#00FFFF' : '#0088aa', fontWeight: 700, fontSize: 18, letterSpacing: '0.05em', textShadow: mapTheme === 'dark' ? '0 0 10px #00FFFF' : 'none', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ position: 'relative', flex: 1, maxWidth: 460 }}>
          <input
            ...
            style={{
              ...
              opacity: drawMode ? 0.4 : 1, cursor: drawMode ? 'not-allowed' : 'text',
            }}
          />
        ...
        {/* Botão Modo Desenho Livre */}
        <button onClick={toggleDrawMode} disabled={!drawReady}
          title={drawReady ? 'Ativar/desativar desenho manual de polígono' : 'Carregando ferramenta de desenho...'}
          style={{
            padding: '9px 14px', whiteSpace: 'nowrap',
            background: drawMode ? hexToRgba('#FF9900', 0.2) : (mapTheme === 'dark' ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)'),
            border: drawMode ? '1px solid rgba(255,153,0,0.5)' : (mapTheme === 'dark' ? '1px solid rgba(255,255,255,0.25)' : '1px solid rgba(0,0,0,0.15)'),
            borderRadius: 8, color: drawMode ? '#FF9900' : (mapTheme === 'dark' ? '#ccc' : '#444'),
            cursor: drawReady ? 'pointer' : 'wait', fontSize: 13, fontWeight: 600, flexShrink: 0,
            opacity: drawReady ? 1 : 0.5,
          }}>
          ✏️ {drawMode ? 'Desenho Ativo' : 'Modo Desenho'}
        </button>
      ...
      {/* ── Aviso do Modo Desenho ── */}
      {drawMode && (
        <div style={{
          position: 'absolute', top: 72, left: '50%', transform: 'translateX(-50%)', zIndex: 700,
          background: hexToRgba('#FF9900', 0.15), border: '1px solid rgba(255,153,0,0.5)',
          borderRadius: 8, padding: '8px 16px', color: '#FF9900', fontSize: 12, fontWeight: 600,
          backdropFilter: 'blur(8px)', maxWidth: 420, textAlign: 'center',
        }}>
          ✏️ {drawMsg || 'Modo Desenho Livre ativo'}
        </div>
      )}

      {/* ── Painel de Resumo Territorial (bairros da cidade buscada) ── */}
      {territorioPanel !== null && !drawMode && (
        <div style={{
          position: 'absolute', top: 72, right: 16, zIndex: 600, width: 300, maxHeight: '46vh',
          ...
          <div style={{ overflowY: 'auto', flex: 1, padding: 8 }}>
            {territorioLoading && (
              <div style={{ padding: '16px', textAlign: 'center', color: mapTheme === 'dark' ? '#888' : '#999', fontSize: 12 }}>⟳ Consultando registros e desenhando no mapa...</div>
            )}
          ...
          {!territorioLoading && territorioPanel.length > 0 && (
            <div style={{ padding: '8px 14px', borderTop: mapTheme === 'dark' ? '1px solid rgba(255,255,255,0.06)' : '1px solid rgba(0,0,0,0.06)', fontSize: 10, color: mapTheme === 'dark' ? '#555' : '#999' }}>
              {bairrosCarregados} de {territorioPanel.length} {territorioPanel.length === 1 ? 'área desenhada no mapa' : 'áreas desenhadas no mapa'} · clique para focar
            </div>
          )}
        </div>
      )}

      {/* ── Legenda Fixa (todas as siglas e cores em uso no banco) ── */}
      {!drawMode && (
        <div style={{
          position: 'absolute',
          top: territorioPanel !== null ? 'calc(72px + 46vh + 12px)' : 72,
          right: 16, zIndex: 590, width: 220,
          background: mapTheme === 'dark' ? 'rgba(8,8,18,0.95)' : 'rgba(255,255,255,0.97)',
          border: mapTheme === 'dark' ? '1px solid rgba(0,255,255,0.2)' : '1px solid rgba(0,0,0,0.1)',
          borderRadius: 10, backdropFilter: 'blur(12px)', boxShadow: '0 6px 20px rgba(0,0,0,0.35)',
          display: 'flex', flexDirection: 'column', overflow: 'hidden', transition: 'top 0.3s',
        }}>
          <div onClick={() => setLegendaAberta(v => !v)}
            style={{
              padding: '9px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer',
              borderBottom: legendaAberta ? (mapTheme === 'dark' ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(0,0,0,0.08)') : 'none',
            }}>
            <div style={{ color: mapTheme === 'dark' ? '#00FFFF' : '#0088aa', fontWeight: 700, fontSize: 11, letterSpacing: '0.05em' }}>
              🗺 LEGENDA
            </div>
            <span style={{ color: mapTheme === 'dark' ? '#666' : '#999', fontSize: 11 }}>{legendaAberta ? '▾' : '▸'}</span>
          </div>

          {legendaAberta && (
            <div style={{ overflowY: 'auto', padding: 8, maxHeight: 240 }}>
              {legendaLoading && (
                <div style={{ padding: '10px', textAlign: 'center', color: mapTheme === 'dark' ? '#666' : '#999', fontSize: 11 }}>⟳ Carregando...</div>
              )}
              {!legendaLoading && legendaDados.length === 0 && (
                <div style={{ padding: '10px', textAlign: 'center', color: mapTheme === 'dark' ? '#555' : '#999', fontSize: 11 }}>
                  Nenhuma sigla cadastrada ainda.
                </div>
              )}
              {!legendaLoading && legendaDados.map(item => {
                const cor = faccaoColor(item.faccao) || '#888';
                return (
                  <div key={item.faccao} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 4px' }}>
                    <div style={{ width: 12, height: 12, borderRadius: 3, background: cor, boxShadow: `0 0 5px ${cor}`, flexShrink: 0 }} />
                    <span style={{ fontSize: 11, fontWeight: 700, color: cor, flexShrink: 0 }}>{item.faccao.toUpperCase()}</span>
                    <span style={{ fontSize: 10, color: mapTheme === 'dark' ? '#666' : '#999', marginLeft: 'auto', flexShrink: 0 }}>
                      {item.total} {item.total === 1 ? 'área' : 'áreas'}
                    </span>
                  </div>
                );
              })}
      ...
      {/* ── Métricas + última atualização da área atual ── */}
      {metrics && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {hasDrawnShape && (
              <span style={{ fontSize: 10, color: '#FF9900', fontWeight: 700 }}>✏️ desenho manual</span>
            )}
            {areaData.faccao && (
              <span style={{
          ...
          {currentUpdatedAt && (
            <div style={{ fontSize: 10, color: mapTheme === 'dark' ? '#888' : '#777' }}>
              🕒 Atualizado {formatRelativeTime(currentUpdatedAt)} · {formatDateTime(currentUpdatedAt)}
            </div>
          )}
        </div>
      )}
        ...
        <div style={{ padding: '72px 16px 16px', overflowY: 'auto', flex: 1, minWidth: 340, display: 'flex', flexDirection: 'column' }}>
          ...
          {activeTab === 'style' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              {/* Bloco de Desenho Manual */}
              <SectionTitle theme={mapTheme}>Delimitação Manual</SectionTitle>
              <div style={{
                padding: '10px 12px', borderRadius: 8,
                background: drawMode ? hexToRgba('#FF9900', 0.08) : (mapTheme === 'dark' ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)'),
                border: drawMode ? '1px solid rgba(255,153,0,0.3)' : (mapTheme === 'dark' ? '1px solid rgba(255,255,255,0.06)' : '1px solid rgba(0,0,0,0.06)'),
                display: 'flex', flexDirection: 'column', gap: 8,
              }}>
                <div style={{ fontSize: 11, color: mapTheme === 'dark' ? '#999' : '#666', lineHeight: 1.4 }}>
                  {drawReady
                    ? 'Desenhe um polígono livremente no mapa, ou edite os vértices de uma área já buscada.'
                    : '⟳ Carregando ferramenta de desenho...'}
                </div>

                <button onClick={toggleDrawMode} disabled={!drawReady}
                  style={btnStyle(drawMode ? '#3a2200' : '#1a1a0a', '#FF9900', !drawReady)}>
                  {drawMode ? '✕ Encerrar Modo Desenho' : '✏️ Ativar Modo Desenho'}
                </button>

                {!drawMode && currentGeometry && (
                  <button onClick={editarPoligonoAtual} disabled={!drawReady}
                    style={btnStyle('#1a1a0a', '#FFD700', !drawReady)}>
                    🖊 Editar Vértices da Área Atual
                  </button>
                )}

                {hasDrawnShape && (
                  <button onClick={clearDrawnShape} style={btnStyle('#2a0a0a', '#FF6666')}>
                    🗑 Limpar Desenho
                  </button>
                )}
              </div>
              ...
              <ToggleRow label="🗺 Mostrar Legenda" value={legendaAberta} onChange={setLegendaAberta} color={style.cor} theme={mapTheme} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              {hasDrawnShape && (
                <div style={{
                  ...
                  background: hexToRgba('#FF9900', 0.08), color: '#FF9900',
                  border: '1px solid rgba(255,153,0,0.25)',
                }}>
                  ✏️ Esta área foi delimitada manualmente (desenho livre)
                </div>
              )}

              {currentAreaId && currentUpdatedAt && (
                <div style={{
                  padding: '8px 12px', borderRadius: 6, fontSize: 11,
                  background: mapTheme === 'dark' ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.03)',
                  color: mapTheme === 'dark' ? '#888' : '#777',
                  border: mapTheme === 'dark' ? '1px solid rgba(255,255,255,0.06)' : '1px solid rgba(0,0,0,0.06)',
                }}>
                  🕒 Última atualização: <strong style={{ color: mapTheme === 'dark' ? '#bbb' : '#444' }}>{formatDateTime(currentUpdatedAt)}</strong>
                  <span style={{ opacity: 0.7 }}> ({formatRelativeTime(currentUpdatedAt)})</span>
                </div>
              )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, flex: 1, minHeight: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                <button onClick={toggleSelectMode} style={{
                  ...
                  background: selectMode ? hexToRgba('#FF5555', 0.15) : (mapTheme === 'dark' ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)'),
                  border: selectMode ? '1px solid rgba(255,85,85,0.4)' : (mapTheme === 'dark' ? '1px solid rgba(255,255,255,0.12)' : '1px solid rgba(0,0,0,0.12)'),
                  color: selectMode ? '#FF5555' : (mapTheme === 'dark' ? '#888' : '#666'),
                  fontSize: 12, cursor: 'pointer', whiteSpace: 'nowrap',
                }}>
                  {selectMode ? '✕ Cancelar' : '🗑 Limpar Lista'}
                </button>
              </div>
              ...
              {selectMode && (
                <div style={{
                  ...
                  background: mapTheme === 'dark' ? 'rgba(255,85,85,0.08)' : 'rgba(255,85,85,0.06)',
                  border: '1px solid rgba(255,85,85,0.25)',
                }}>
                  <span style={{ fontSize: 11, color: mapTheme === 'dark' ? '#ff8888' : '#cc3333', flex: 1 }}>
                    {selectedIds.size} selecionada{selectedIds.size !== 1 ? 's' : ''}
                  </span>
                  <button onClick={selectAllVisible} style={microBtn('#1a1a1a', '#888')}>Todas</button>
                  <button onClick={clearSelection} style={microBtn('#1a1a1a', '#888')}>Nenhuma</button>
                  <button
                    onClick={handleBulkDelete}
                    disabled={selectedIds.size === 0 || bulkDeleting}
                    style={{
                      ...microBtn(confirmBulkDelete ? '#440a0a' : '#2a0a0a', confirmBulkDelete ? '#FF5555' : '#FF8888'),
                      opacity: selectedIds.size === 0 ? 0.4 : 1,
                      cursor: selectedIds.size === 0 ? 'not-allowed' : 'pointer',
                    }}>
                    {bulkDeleting ? '⟳...' : confirmBulkDelete ? '⚠ Confirmar exclusão' : `🗑 Excluir (${selectedIds.size})`}
                  </button>
                </div>
              )}
                ...
                {filteredAreas.map(area => {
                  ...
                  return (
                    <div key={area.id}
                      style={{
                        borderRadius: 8,
                        border: isSelected ? '1px solid #FF5555' : currentAreaId === area.id ? `1px solid ${area.cor_contorno}` : (mapTheme === 'dark' ? '1px solid rgba(255,255,255,0.07)' : '1px solid rgba(0,0,0,0.08)'),
                        background: isSelected ? 'rgba(255,85,85,0.08)' : currentAreaId === area.id ? hexToRgba(area.cor_contorno, 0.1) : (mapTheme === 'dark' ? 'rgba(255,255,255,0.025)' : 'rgba(0,0,0,0.02)'),
                        overflow: 'hidden',
                      }}
                    >
                      <div onClick={() => selectMode ? toggleSelectId(area.id) : handleLoadSavedArea(area)}
                        style={{ padding: '10px 12px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10 }}>
                        {selectMode && (
                          <input type="checkbox" checked={isSelected} onChange={() => toggleSelectId(area.id)}
                            onClick={e => e.stopPropagation()}
                            style={{ accentColor: '#FF5555', width: 14, height: 14, flexShrink: 0, cursor: 'pointer' }} />
                        )}
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ color: mapTheme === 'dark' ? '#444' : '#aaa', fontSize: 10, marginTop: 2 }}>
                            🕒 {formatRelativeTime(area.data_atualizacao || area.data_criacao)}
                          </div>
                        </div>
                      </div>
      ...
      <style>{`
        ...
        /* Leaflet.draw — adapta a toolbar ao tema escuro do app */
        /* Eleva o z-index do controle (Leaflet usa 800 por padrão) para ficar
           acima da nossa barra superior (zIndex 1000 cobre top:0/left:0/right:0).
           O deslocamento horizontal (esquerda) para não ficar atrás do painel
           lateral (340px) é feito via JS no useEffect que observa sidebarOpen. */
        ...
        .leaflet-draw-toolbar a {
          ...
          /* O ícone do plugin é um SVG escuro pensado para fundo claro.
             Invertemos para ficar claro, mas sem saturar o brilho —
             excesso de brightness apaga o desenho do ícone, deixando
             apenas um quadrado branco em vez do símbolo da ferramenta. */
          filter: invert(0.85);
        }
        .leaflet-draw-toolbar a:hover {
          filter: invert(0.85) brightness(1.15);
        }
        /* Cores de identificação por ferramenta, para reconhecer de imediato
           sem precisar passar o mouse e esperar o tooltip */
        ...
        .leaflet-draw-toolbar .leaflet-draw-edit-edit {
          background-color: #332a14 !important;
        ...
        .leaflet-draw-actions a { background-color: #1a1a1a !important; color: #ddd !important; }
        .leaflet-draw-actions a:hover { background-color: #2a2a2a !important; }
        .leaflet-draw-tooltip { background: rgba(0,0,0,0.85) !important; border-color: rgba(255,153,0,0.4) !important; }
        .leaflet-draw-tooltip:before { border-right-color: rgba(0,0,0,0.85) !important; }
      `}</style>
    </div>
  );
}
