...
export default function App() {
  ...
  const mainLayerRef    = useRef<any>(null);  // camada do polígono principal (cidade ou bairro selecionado)
  const bairrosLayerRef = useRef<any>(null);  // camada dos bairros faccionados sobrepostos
  ...
  useEffect(() => {
    ...
    // Duas camadas: principal (cidade/bairro selecionado) e bairros faccionados sobrepostos
  ...
  // Cada bairro usa sua própria cor (cor_contorno salva), com brilho mais sutil
  // para não competir visualmente com o contorno principal da cidade.
  const drawBairrosSobrepostos = useCallback(async (areas: SavedArea[]) => {
    ...
    // Busca a geometria completa de cada bairro (a listagem não traz geometry)
    ...
    results.forEach((res, idx) => {
      ...
      // Glow sutil para destacar sem competir com o polígono principal
      L.geoJSON(feature, { style: () => ({ color: cor, weight: 9, opacity: 0.12, fill: false }) }).addTo(lg);

      // Popup com nome + sigla ao clicar no bairro
  ...
  const fetchTerritorioDaCidade = useCallback(async (municipioNome: string) => {
    ...
    try {
      ...
      // Sobrepõe os bairros encontrados no mapa, junto com o contorno principal
  ...
  const handleSelect = async (result: SearchResult) => {
    ...
    // Consulta e sobrepõe os bairros faccionados depois de desenhar a cidade
  ...
  const handleLoadSavedArea = async (area: SavedArea) => {
    ...
    try {
      ...
      // Se o bairro carregado tem município, também sobrepõe os "vizinhos" faccionados
        ...
        <div style={{ padding: '72px 16px 16px', overflowY: 'auto', flex: 1, minWidth: 340, display: 'flex', flexDirection: 'column' }}>
          ...
          {activeTab === 'list' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, flex: 1, minHeight: 0 }}>
              ...
              {/* Recarregar + Toggle de seleção múltipla */}
