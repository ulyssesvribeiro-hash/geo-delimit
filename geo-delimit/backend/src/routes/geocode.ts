...
router.get('/polygon', async (req: Request, res: Response) => {
  try {
    ...
    // Tenta múltiplos servidores Overpass até um funcionar
    for (const baseUrl of OVERPASS_URLS) {
      try {
        console.log('[geocode/polygon] Tentando servidor:', baseUrl);
        ...
        console.log('[geocode/polygon] Status:', overpassResponse.status, 'de', baseUrl);

        if (!overpassResponse.ok) {
          // Não tenta ler o body de novo depois — só loga o status e tenta o próximo servidor
          console.error('[geocode/polygon] Servidor retornou erro, tentando próximo...');
          continue;
        }

        overpassData = await overpassResponse.json() as OverpassResult;
        if (overpassData?.elements?.length > 0) {
          console.log('[geocode/polygon] Sucesso com:', baseUrl);
          break;
        }
      } catch (innerErr) {
        console.error('[geocode/polygon] Falha ao tentar', baseUrl, ':', innerErr instanceof Error ? innerErr.message : innerErr);
        continue;
      }
    }

    const geojson = overpassData ? overpassToGeoJSON(overpassData) : null;

    if (!geojson) {
      console.log('[geocode/polygon] Overpass falhou em todos os servidores, tentando Nominatim details...');
